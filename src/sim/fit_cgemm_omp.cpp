...
void CGemmType::read(Token& token){
    ...
    if(rvdw_<0) throw std::invalid_argument("CGemmType::read(Token&): invalid rvdw.");
...
double objf(const std::vector<double> &x, std::vector<double> &grad, void* fData){
    ...
    double error=0;
    double np=0;
    #pragma omp parallel for reduction(+:error,t_avg,f_avg,np) reduction(max:t_max,f_max)
    for(int n=0; n<fData_.data.size(); ++n){
        Engine engineloc=engine;
        for(int i=0; i<engineloc.calcs().size(); ++i){
            engineloc.calc(i)=copy(engine.calc(i));
        }
        for(int i=0; i<engineloc.constraints().size(); ++i){
            engineloc.constraint(i)=copy(engine.constraint(i));
        }
        ...
        engineloc.constraints().front()->indices()=indices;
        ...
        if(nShells<nCores){
            ...
            for(int j=nCores; j<nShells; ++j){
                ...
                strucN.posn(nCores+j).noalias()+=1.0*dev*Eigen::Vector3d::Random();
            }
        ...
        if(PRINT_STRUC>0){
            ...
            for(int m=0; m<strucN.nAtoms(); ++m){
                std::cout<<strucN.name(m)
                    ...
                    <<" "<<fData_.types[strucN.type(m)].rvdw()
        ...
        for(t=0; t<fData_.nsteps; ++t){
            //compute step
            fData_.intg->compute(strucN,engineloc);
            //compute total force
            ftot=0;
            for(int m=0; m<strucN.nAtoms(); ++m){
...
int main(int argc, char* argv[]){
    ...
    try{
        ...
        if(rvdwc<=0.0) throw std::invalid_argument("Error in fit_cgemm_omp(int,char**): Invalid rvdwc.");
        ...
        #pragma omp parallel
        {
            ...
            if(thread_id==0) std::cout<<"nthreads = "<<num_threads<<"\n";
        }
		std::cout<<print::buf(strbuf)<<"\n";
        ...
        for(int i=0; i<types.size(); ++i){
            ...
            if(types[i].rvdw()<=0.0) throw std::invalid_argument("Invalid rvdw.");
        ...
        if(eIndex<0) throw std::invalid_argument("No electron element name \"X\" found in type list.");
