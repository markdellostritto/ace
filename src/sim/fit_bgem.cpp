...
std::ostream& operator<<(std::ostream& out, const BGemType& type){
    return out<<type.name()<<" "<<type.index()<<" "<<type.mass()<<" "<<type.alpha()<<" "<<type.amp()<<" "<<type.rep();
}

//==== member functions ====

void BGemType::read(Token& token){
    name_=token.next();
    mass_=std::atof(token.next().c_str());
    alpha_=std::atof(token.next().c_str());
    amp_=std::atof(token.next().c_str());
    rep_=std::atof(token.next().c_str());
    if(alpha_<0) throw std::invalid_argument("BGemType::read(Token&): invalid alpha.");
    //if(amp_<0) throw std::invalid_argument("BGemType::read(Token&): invalid amplitude.");
}
...
double objf(const std::vector<double> &x, std::vector<double> &grad, void* fData){
    ...
    CalcBGemCut& calcBGemCut=static_cast<CalcBGemCut&>(*fData_.engine.calcs().front());
    c=0;
    if(opt_alpha) for(int i=0; i<ntypes; ++i) calcBGemCut.alpha()[i]=x[c++];
    else for(int i=0; i<ntypes; ++i) calcBGemCut.alpha()[i]=fData_.types[i].alpha();
    if(opt_amp) for(int i=0; i<ntypes; ++i) calcBGemCut.amp()(i,i)=x[c++];
    else for(int i=0; i<ntypes; ++i) calcBGemCut.amp()(i,i)=fData_.types[i].amp();
    for(int i=0; i<ntypes; ++i) calcBGemCut.rep()(i,i)=fData_.types[i].rep();
    calcBGemCut.init();
    c=0;
    if(opt_alpha) for(int i=0; i<ntypes; ++i) fData_.types[i].alpha()=x[c++];
    if(opt_amp) for(int i=0; i<ntypes; ++i) fData_.types[i].amp()=x[c++];
    //std::cout<<calcBGemCut<<"\n";
    //std::cout<<"alpha = \n"<<calcBGemCut.alpha()<<"\n";
    //std::cout<<"amp = \n"<<calcBGemCut.amp()<<"\n";
        ...
        if(PRINT_STRUC>0){
            ...
            for(int m=0; m<strucN.nAtoms(); ++m){
                std::cout<<strucN.name(m)
                    <<" "<<strucN.type(m)
                    <<" "<<strucN.charge(m)
                    <<" "<<fData_.types[strucN.type(m)].alpha()
                    <<" "<<fData_.types[strucN.type(m)].amp()
            ...
            for(int j=0; j<npts[1]; ++j){
                for(int k=0; k<npts[2]; ++k){
                    ...
                    for(int m=0; m<strucN.nAtoms(); ++m){
                        ...
                        const double salpha=std::sqrt(calcBGemCut.alpha()[strucN.type(m)]);
                        data+=pf*strucN.charge(m)/dr*std::erf(salpha*dr);
                    ...
                    //std::cout<<"espN = "<<espN.data(i,j,k)<<"\n";
                    //std::cout<<"espA = "<<espA.data(i,j,k)<<"\n";
                }
            }
        }
...
int main(int argc, char* argv[]){
    ...
    //function data
        ...
        std::vector<BGemType>& types=functionData.types;
    ...
    try{
		...
		while(fgets(input,string::M,reader)!=NULL){
            ...
            } else if(tag=="OPT_ALPHA"){
				opt_alpha=string::boolean(token.next().c_str());
			} else if(tag=="OPT_AMP"){
				opt_amp=string::boolean(token.next().c_str());
			} 
        }
        ...
        engine.calcs().push_back(
            std::make_shared<CalcBGemCut>(rc)
        );
        std::cout<<"test1\n";
        ...
        //==== print ====
        int dim=0;
        if(opt_alpha) dim+=ntypes;
        if(opt_amp) dim+=ntypes;
        ...
        std::cout<<"opt_alpha = "<<opt_alpha<<"\n";
        std::cout<<"opt_amp   = "<<opt_amp<<"\n";
        ...
        for(int i=0; i<types.size(); ++i){
            ...
            if(types[i].alpha()<=0.0) throw std::invalid_argument("Invalid alpha.");
            //if(types[i].amp()<=0.0) throw std::invalid_argument("Invalid amplitude.");
        }
        ...
        c=0;
        if(opt_alpha) for(int i=0; i<ntypes; ++i) lb[c++]=1.0e-6;
        if(opt_amp) for(int i=0; i<ntypes; ++i) lb[c++]=1.0e-6;
        c=0;
        if(opt_alpha) for(int i=0; i<ntypes; ++i) ub[c++]=HUGE_VAL;
        if(opt_amp) for(int i=0; i<ntypes; ++i) ub[c++]=HUGE_VAL;
        c=0;
        //if(opt_alpha) for(int i=0; i<ntypes; ++i) x[c++]=types[i].alpha();
        //if(opt_amp) for(int i=0; i<ntypes; ++i) x[c++]=types[i].amp();
        if(opt_alpha) for(int i=0; i<ntypes; ++i) x[c++]=dist_pos(rngen);
        if(opt_amp) for(int i=0; i<ntypes; ++i) x[c++]=dist_pos(rngen);
        ...
        for(int n=0; n<data.size(); ++n){
            ...
            CalcBGemCut& calcBGemCut=static_cast<CalcBGemCut&>(*engine.calcs().back());
            const Eigen::Vector3i& npts=espA.n();
            const double pf=1.0/(4.0*PI*units::Consts::eps0());
            //std::cout<<"origin = "<<espN.origin().transpose()<<"\n";
            for(int i=0; i<npts[0]; ++i){
                for(int j=0; j<npts[1]; ++j){
                    for(int k=0; k<npts[2]; ++k){
                        ...
                        for(int m=0; m<strucN.nAtoms(); ++m){
                            const double dr=(r-strucN.posn(m)).norm();
                            const double alpha=calcBGemCut.alpha()[strucN.type(m)];
