...
* @param dr - the triple distances: dr={rij,rik,rjk} with i at the vertex
*/
void BasisA::symm(double drIJ, double drIK, double cosIJK){
	if(BASIS_ANGULAR_PRINT_FUNC>0) std::cout<<"BasisA::symm(double,const double*):\n";
	const double cprod=cutoff_.cutf(drIJ)*cutoff_.cutf(drIK);//cut(rij)*cut(rik)
	const double r2s=drIJ*drIJ+drIK*drIK;
	switch(name_){
		case BasisA::Name::GAUSS:{
			for(int j=0; j<etar_.size(); ++j) fdampr_[j]=cprod*fmexp(-ietar2_[j]*r2s);
			for(int i=0; i<size_; ++i){
				//symm_[i]=cprod*pow(fabs(0.5+lambdaf_[i]*cosIJK),zeta_[i])*fmexp(-ieta2_[i]*r2s);
				symm_[i]=pow(fabs(0.5+lambdaf_[i]*cosIJK),zeta_[i])*fdampr_[rflag_[i]];
			}
		} break;
		case BasisA::Name::GAUSS2:{
			...
			for(int i=0; i<size_; ++i){
				//symm_[i]=cprod*pow(fabs(0.5+lambdaf_[i]*cosIJK),zeta_[i])*fmexp(-ieta2_[i]*r2s);
				symm_[i]=cprod*pow(fabs(0.5+lambdaf_[i]*cosIJK),zeta_[i])*2.0*fdampr_[rflag_[i]]/(1.0+fdampr_[rflag_[i]]);
			}
		} break;
		case BasisA::Name::SECH:{
			for(int j=0; j<etar_.size(); ++j) fdampr_[j]=fmexp(-0.5*PI*ietar2_[j]*r2s);
			for(int i=0; i<size_; ++i){
				...
				symm_[i]=cprod*pow(fabs(0.5+lambdaf_[i]*cosIJK),zeta_[i])*2.0*fexp/(1.0+fexp*fexp);
			}
		} break;
		case BasisA::Name::STUDENT3:{
...
* @param dEdG - gradient of energy w.r.t. the inputs
*/
void BasisA::force(double drIJ, double drIK, double cosIJK, double& phi, double* eta, const double* dEdG){
	//compute cutoffs
	const double c[2]={
		cutoff_.cutf(drIJ),//cut(rij)
		cutoff_.cutf(drIK)//cut(rik)
	};
	const double g[2]={
		cutoff_.cutg(drIJ),//cut'(rij)
		cutoff_.cutg(drIK)//cut'(rik)
	};
	//compute phi, eta
	phi=0;
	eta[0]=0;
	...
	switch(name_){
		case BasisA::Name::GAUSS:{
			const double dij=2.0*drIJ*c[0];
			const double dik=2.0*drIK*c[1];
			for(int j=0; j<etar_.size(); ++j) fdampr_[j]=fmexp(-ietar2_[j]*r2s);
			for(int i=0; i<size_; ++i){
				//compute angular values
				const double cw=fabs(0.5+lambdaf_[i]*cosIJK);
		...
		case BasisA::Name::GAUSS2:{
			const double dij=PI*drIJ*c[0];
			const double dik=PI*drIK*c[1];
			for(int j=0; j<etar_.size(); ++j) fdampr_[j]=fmexp(-0.5*PI*ietar2_[j]*r2s);
			for(int i=0; i<size_; ++i){
				const double fexp=fdampr_[rflag_[i]];
				const double den=1.0/(1.0+fexp);
		...
		case BasisA::Name::STUDENT3:{
			const double dij=4.0*drIJ*c[0];
			const double dik=4.0*drIK*c[1];
		...
		case BasisA::Name::STUDENT4:{
			const double dij=5.0*drIJ*c[0];
			const double dik=5.0*drIK*c[1];
			for(int j=0; j<etar_.size(); ++j) fdampr_[j]=1.0/sqrt(1.0+ietar2_[j]*r2s);
			for(int i=0; i<size_; ++i){
				//compute angular values
				const double den=fdampr_[rflag_[i]];
		...
		case BasisA::Name::STUDENT5:{
			const double dij=6.0*drIJ*c[0];
			const double dik=6.0*drIK*c[1];
			for(int j=0; j<etar_.size(); ++j) fdampr_[j]=1.0/sqrt(1.0+ietar2_[j]*r2s);
			for(int i=0; i<size_; ++i){
				//compute angular values
				const double den=fdampr_[rflag_[i]];
