...
* @param struc_val - list of structures used for validation
*/
void NNPTE::error_cost(const Eigen::VectorXd& x, const std::vector<Structure>& struc_train, const std::vector<Structure>& struc_val){
...
* given a set of parameters for a NNP, compute the error associated with a given set of structures
* the error is computed using the cost function associated with nnpte
* it is further assumed that the network has only a single ouput
* this allows for optimizations which improve computational efficiency
* @param x - parameters (weights+biases) of a neural network potential
* @param struc_train - list of structures used for training
* @param struc_val - list of structures used for validation
*/
void NNPTE::error_cost_o1(const Eigen::VectorXd& x, const std::vector<Structure>& struc_train, const std::vector<Structure>& struc_val){
	if(NNPTE_PRINT_FUNC>0) std::cout<<"NNPTE::error(const Eigen::VectorXd&):\n";
...
/**
* given a set of parameters for a NNP, compute the error associated with a given set of structures
* the error is computed by directly computing the gradient of the output w.r.t the parameters
* @param x - parameters (weights+biases) of a neural network potential
* @param struc_train - list of structures used for training
* @param struc_val - list of structures used for validation
*/
void NNPTE::error_dodp(const Eigen::VectorXd& x, const std::vector<Structure>& struc_train, const std::vector<Structure>& struc_val){
	if(NNPTE_PRINT_FUNC>0) std::cout<<"NNPTE::error(const Eigen::VectorXd&):\n";
		...
		if(compute.rep){
			...
			for(int n=0; n<3; ++n){
				...
				for(int i=BATCH.rank(); i<dist[n].size(); i+=BATCH.size()){
					...
					erep[i]=pot_rep.energy(strucs[n][i],nlist);
				}
				MPI_Allreduce(MPI_IN_PLACE,erep.data(),erep.size(),MPI_DOUBLE,MPI_MIN,BATCH.mpic());
				for(int i=0; i<dist[n].size(); ++i){
					strucs[n][i].erep()=erep[i];
					strucs[n][i].pe()-=erep[i];
				}
			}
		}
