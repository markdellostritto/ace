...
template <> int nbytes(const NNPTE& obj){
   if(NNPTE_PRINT_FUNC>0) std::cout<<"nbytes(const NNPTE&)\n";
	...
	//optimization
		...
		size+=sizeof(double);//eweight_
		size+=sizeof(double);//fweight_
...
template <> int pack(const NNPTE& obj, char* arr){
	...
	//optimization
		...
		std::memcpy(arr+pos,&obj.eweight_,sizeof(double)); pos+=sizeof(double);
		std::memcpy(arr+pos,&obj.fweight_,sizeof(double)); pos+=sizeof(double);
...
template <> int unpack(NNPTE& obj, const char* arr){
	...
	//optimization
		...
		std::memcpy(&obj.eweight_,arr+pos,sizeof(double)); pos+=sizeof(double);
		std::memcpy(&obj.fweight_,arr+pos,sizeof(double)); pos+=sizeof(double);
...
std::ostream& operator<<(std::ostream& out, const NNPTE& nnpte){
	...
	out<<"eweight      = "<<nnpte.eweight()<<"\n";
	out<<"fweight      = "<<nnpte.fweight()<<"\n";
	out<<"eta          = "<<nnpte.eta()<<"\n";
	out<<print::buf(str);
	delete[] str;
	return out;
...
void NNPTE::train(int batchSize){
	...
	if(struc_train_ref_.size()==0) throw std::invalid_argument("NNPTE::train(int): No training data provided.");
	if(struc_val_ref_.size()==0) throw std::invalid_argument("NNPTE::train(int): No validation data provided.");
	struc_train_nnp_=struc_train_ref_;
	struc_val_nnp_=struc_val_ref_;
	struc_train_rp_=struc_train_ref_;
	struc_train_rm_=struc_train_ref_;
	...
	double nTrainF=(1.0*struc_train_ref_.size())/BATCH.size();
	double nValF=(1.0*struc_val_ref_.size())/BATCH.size();
	...
	if(NNPTE_PRINT_STATUS>0 && WORLD.rank()==0) std::cout<<"generating neighbor lists\n";
	...
	dist_atomt.resize(struc_train_ref_.size());
	dist_atomv.resize(struc_val_ref_.size());
	for(int i=0; i<struc_train_ref_.size(); ++i) dist_atomt[i].init(BATCH.size(),BATCH.rank(),struc_train_ref_[i].nAtoms());
	for(int i=0; i<struc_val_ref_.size(); ++i) dist_atomv[i].init(BATCH.size(),BATCH.rank(),struc_val_ref_[i].nAtoms());
	normt_.resize(struc_train_ref_.size());
	normv_.resize(struc_val_ref_.size());
	...
	batcht_.resize(batchSize,struc_train_ref_.size());
	batchv_.resize(batchSize,struc_val_ref_.size());
...
void NNPTE::error2(const Eigen::VectorXd& x){
	...
	if(batcht_.count()>=batcht_.capacity()){
		...
		MPI_Bcast(batcht_.data(),batcht_.capacity(),MPI_INT,0,BATCH.mpic());
		batcht_.count()=0;
	}
	...
	for(int bi=0; bi<batcht_.size(); ++bi){
		//**** set the structure index ****
		const int si=batcht_[bi];
		
		//**** compute the energy and forces ****
		std::vector<double> dr(2);
		std::vector<double> cos(1);
		//reset
		struc_train_nnp_[si].pe()=0;
		for(int n=0; n<dist_atomt[si].size(); ++n){
			const int i=dist_atomt[si].index(n);
			struc_train_nnp_[si].force(i).setZero();
		}
		//compute
		for(int n=0; n<dist_atomt[si].size(); ++n){
			//get index and type
			...
			struc_train_nnp_[si].pe()+=nnp_.nnh(II).nn().out()[0];
			...
			for(int j=0; j<nlist_train_[si].size(i); ++j){
				...
				const double amp=nnp_.nnh(II).basisR(JJ).force(dr,dEdG.data()+offsetR_)*dIJi;
				...
				for(int k=j+1; k<nlist_train_[si].size(i); ++k){
					...
					nnp_.nnh(II).basisA(JJ,KK).force(dr,cos,phi,eta,dEdG.data()+offsetA_);
					...
					if(jmin){
						...
						//struc_train_nnp_[si].force(jj).noalias()-=eta[2]*rJK*dJKi;//only for G3, which we don't use
					}
					if(kmin){
						struc_train_nnp_[si].force(kk).noalias()-=(-phi*cosIJK*dIKi+eta[1])*rIK*dIKi+phi*dIKi*rIJ*dIJi;
						//struc_train_nnp_[si].force(kk).noalias()+=eta[2]*rJK*dJKi;//only for G3, which we don't use
					}
				}
			}
		}
		...
		MPI_Allreduce(MPI_IN_PLACE,&struc_train_nnp_[si].pe(),1,MPI_DOUBLE,MPI_SUM,BATCH.mpic());
		
		//**** compute the error - energy ****
		const double nAtoms=struc_train_ref_[si].nAtoms();
		const double dE=(struc_train_nnp_[si].pe()-struc_train_ref_[si].pe());
		const double dU=dE*normt_[si];
		double dcdo=eweight_*normt_[si];
		switch(iter_.loss()){
			case opt::Loss::MSE:{
				error_[0]+=eweight_*0.5*dU*dU;//loss - train
				dcdo*=dU;
			} break;
			case opt::Loss::MAE:{
				error_[0]+=eweight_*std::fabs(dU);//loss - train
				dcdo*=math::special::sgn(dU);
			} break;
			case opt::Loss::HUBER:{
				const double arg=dU*deltai_;
				const double sqrtf=sqrt(1.0+arg*arg);
				error_[0]+=eweight_*delta2_*(sqrtf-1.0);//loss - train
				dcdo*=dU/sqrtf;
			} break;
			case opt::Loss::ASINH:{
				const double arg=dU*deltai_;
				...
				error_[0]+=eweight_*delta2_*(1.0-sqrtf+arg*logf);//loss - train
				dcdo*=logf*delta_;
			} break;
			default: break;
		}
		...
		for(int n=0; n<dist_atomt[si].size(); ++n){
			//get index and type
			const int i=dist_atomt[si].index(n);
			const int II=struc_train_ref_[si].type(i);
			//execute the network
			nnp_.nnh(II).nn().fpbp(struc_train_ref_[si].symm(i));
			//compute the gradient 
			dodp_[II].grad(nnp_.nnh(II).nn());
			gElement_[II].noalias()+=dodp_[II].dodp().row(0)*dcdo;
		}
		
		//**** compute the error - force ****
		double dfmax=0;
		double df2s=0;
		for(int n=0; n<dist_atomt[si].size(); ++n){
			//get index and type
			const int i=dist_atomt[si].index(n);
			//compute error
			const double df2=(struc_train_nnp_[si].force(i)-struc_train_ref_[si].force(i)).squaredNorm();
			df2s+=df2;
			const double dfn=sqrt(df2);
			if(dfn>dfmax) dfmax=dfn;
		}
		df2s*=normt_[si]*normt_[si];
		switch(iter_.loss()){
			case opt::Loss::MSE:{
				error_[0]+=fweight_*0.5*df2s;
			}
			case opt::Loss::MAE:{
				error_[0]+=fweight_*std::sqrt(df2s);
			}
			case opt::Loss::HUBER:{
				error_[0]+=fweight_*delta2_*(sqrt(1.0+df2s/delta2_)-1.0);//loss - train
			}
			case opt::Loss::ASINH:{
		...
		error_[2]+=df2s*fweight_;
		
		//**** compute the gradient - force ****
		//perturb positions
		const double eta=eta_/(dfmax+0.0e-12);
		for(int n=0; n<dist_atomt[si].size(); ++n){
			//get index and type
			const int i=dist_atomt[si].index(n);
			//compute force difference
			const Eigen::Vector3d df=(struc_train_nnp_[si].force(i)-struc_train_ref_[si].force(i));
			//perturb
			struc_train_rp_[si].posn(i).noalias()=struc_train_nnp_[si].posn(i)+eta*df;
			struc_train_rm_[si].posn(i).noalias()=struc_train_nnp_[si].posn(i)-eta*df;
		}
		//compute symmetry functions
		NeighborList nlist;
		nlist.build(struc_train_rp_[si],nnp_.rc());
		NNP::symm(nnp_,struc_train_rp_[si],nlist);
		nlist.build(struc_train_rm_[si],nnp_.rc());
		NNP::symm(nnp_,struc_train_rm_[si],nlist);
		//compute the force gracients
		for(int n=0; n<dist_atomt[si].size(); ++n){
			//get index and type
			const int i=dist_atomt[si].index(n);
			const int II=struc_train_ref_[si].type(i);
			...
			//execute the network
			nnp_.nnh(II).nn().fpbp(struc_train_rp_[si].symm(i));
			//compute dOdP
			dodp_[II].grad(nnp_.nnh(II).nn());
			const Eigen::VectorXd gradp=dodp_[II].dodp().row(0);
			
			//execute the network
			nnp_.nnh(II).nn().fpbp(struc_train_rm_[si].symm(i));
			//compute dOdP
			dodp_[II].grad(nnp_.nnh(II).nn());
			const Eigen::VectorXd gradm=dodp_[II].dodp().row(0);
			
			//force gradient
			switch(iter_.loss()){
				case opt::Loss::MSE:{
					gElement_[II].noalias()+=-fweight_*(gradp-gradm)*0.5/eta;
				}
				case opt::Loss::MAE:{
					gElement_[II].noalias()+=-fweight_*(gradp-gradm)*0.5/eta;
				}
				case opt::Loss::HUBER:{
					gElement_[II].noalias()+=-fweight_*(gradp-gradm)*0.5/eta/std::sqrt(df2s);	
				}
				case opt::Loss::ASINH:{
	...
	if((unsigned int)iter_.step()%iter_.nPrint()==0 || (unsigned int)iter_.step()%iter_.nWrite()==0){
		...
		for(int si=0; si<struc_val_ref_.size(); ++si){
			...
			std::vector<double> cos(1);
			//reset
			struc_val_nnp_[si].pe()=0;
			for(int n=0; n<dist_atomt[si].size(); ++n){
				const int i=dist_atomt[si].index(n);
				struc_val_nnp_[si].force(i).setZero();
			}
			//compute
			for(int n=0; n<dist_atomv[si].size(); ++n){
				//get index and type
				const int i=dist_atomv[si].index(n);
				const int II=struc_val_nnp_[si].type(i);
				//execute the network
				nnp_.nnh(II).nn().fpbp(struc_val_nnp_[si].symm(i));
				//add the atom energy to the total
				struc_val_nnp_[si].pe()+=nnp_.nnh(II).nn().out()[0];
				//calculate the network gradient
				nnp_.nnh(II).dOdZ().grad(nnp_.nnh(II).nn());
				//set the gradient (n.b. dodi - do/di - deriv. of out w.r.t. in)
				const Eigen::VectorXd& dEdG=nnp_.nnh(II).dOdZ().dodi().row(0);
				//loop over all neighbors
				for(int j=0; j<nlist_val_[si].size(i); ++j){
					//get the indices of the jth neighbor
					const int JJ=nlist_val_[si].neigh(i,j).type();
					const int jj=nlist_val_[si].neigh(i,j).index();
					const bool jmin=nlist_val_[si].neigh(i,j).min();
					//get the distance from J to I
					const Eigen::Vector3d& rIJ=nlist_val_[si].neigh(i,j).r();
					const double dIJ=nlist_val_[si].neigh(i,j).dr();
					const double dIJi=1.0/dIJ; dr[0]=dIJ;
					//compute the IJ contribution to the radial force
					const int offsetR_=nnp_.nnh(II).offsetR(JJ);
					const double amp=nnp_.nnh(II).basisR(JJ).force(dr,dEdG.data()+offsetR_)*dIJi;
					struc_val_nnp_[si].force(i).noalias()+=amp*rIJ;
					if(jmin) struc_val_nnp_[si].force(jj).noalias()-=amp*rIJ;
					//loop over all unique triplets
					for(int k=j+1; k<nlist_val_[si].size(i); ++k){
						//find the index of the types of atom k
						const int KK=nlist_val_[si].neigh(i,k).type();
						const int kk=nlist_val_[si].neigh(i,k).index();
						const bool kmin=nlist_val_[si].neigh(i,k).min();
						//get the distance from K to I
						const Eigen::Vector3d& rIK=nlist_val_[si].neigh(i,k).r();
						const double dIK=nlist_val_[si].neigh(i,k).dr();
						const double dIKi=1.0/dIK; dr[1]=dIK;
						//compute the IJ,IK,JK contribution to the angular force
						const int offsetA_=nnp_.nnh(II).nInputR()+nnp_.nnh(II).offsetA(JJ,KK);
						double phi=0; double eta[3]={0,0,0};
						const double cosIJK=rIJ.dot(rIK)*dIJi*dIKi; cos[0]=cosIJK;
						nnp_.nnh(II).basisA(JJ,KK).force(dr,cos,phi,eta,dEdG.data()+offsetA_);
						struc_val_nnp_[si].force(i).noalias()+=(phi*(dIKi-cosIJK*dIJi)+eta[0])*rIJ*dIJi;
						struc_val_nnp_[si].force(i).noalias()+=(phi*(dIJi-cosIJK*dIKi)+eta[1])*rIK*dIKi;
						if(jmin){
							struc_val_nnp_[si].force(jj).noalias()-=(-phi*cosIJK*dIJi+eta[0])*rIJ*dIJi+phi*dIJi*rIK*dIKi;
							//struc_val_nnp_[si].force(jj).noalias()-=eta[2]*rJK*dJKi;//only for G3, which we don't use
						}
						if(kmin){
							struc_val_nnp_[si].force(kk).noalias()-=(-phi*cosIJK*dIKi+eta[1])*rIK*dIKi+phi*dIKi*rIJ*dIJi;
							//struc_val_nnp_[si].force(kk).noalias()+=eta[2]*rJK*dJKi;//only for G3, which we don't use
						}
					}
				}
			}
			...
			//**** accumulate energy ****
			MPI_Allreduce(MPI_IN_PLACE,&struc_val_nnp_[si].pe(),1,MPI_DOUBLE,MPI_SUM,BATCH.mpic());
			
			//**** compute error - energy ****
			const double nAtoms=struc_val_ref_[si].nAtoms();
			const double dE=(struc_val_nnp_[si].pe()-struc_val_ref_[si].pe());
			const double dU=dE*normv_[si];
			switch(iter_.loss()){
				case opt::Loss::MSE:{
					error_[1]+=eweight_*0.5*dU*dU;//loss - val
				} break;
				case opt::Loss::MAE:{
					error_[1]+=eweight_*std::fabs(dU);//loss - val
				} break;
				case opt::Loss::HUBER:{
					const double arg=dU*deltai_;
					error_[1]+=eweight_*delta2_*(sqrt(1.0+arg*arg)-1.0);//loss - val
				} break;
				case opt::Loss::ASINH:{
					const double arg=dU*deltai_;
					const double sqrtf=sqrt(1.0+arg*arg);
					error_[1]+=eweight_*delta2_*(1.0-sqrtf+arg*log(arg+sqrtf));//loss - val
				} break;
				default: break;
			}
			error_[3]+=eweight_*(dE*dE)/(nAtoms*nAtoms);//rmse - val
			...
			for(int n=0; n<dist_atomv[si].size(); ++n){
				...
				const int i=dist_atomv[si].index(n);
				//compute error
				df2s+=(struc_val_nnp_[si].force(i)-struc_val_ref_[si].force(i)).squaredNorm();
			}
			df2s*=normv_[si]*normv_[si];
			switch(iter_.loss()){
				case opt::Loss::MSE:{
					error_[1]+=fweight_*0.5*df2s;
				}
				case opt::Loss::MAE:{
					error_[1]+=fweight_*std::sqrt(df2s);
				}
				case opt::Loss::HUBER:{
					error_[1]+=fweight_*delta2_*(sqrt(1.0+df2s/delta2_)-1.0);//loss - train
				}
				case opt::Loss::ASINH:{
...
void NNPTE::error3(const Eigen::VectorXd& x){
	if(NNPTE_PRINT_FUNC>0) std::cout<<"NNPTE::error(const Eigen::VectorXd&):\n";
	...
	for(int i=0; i<batcht_.size(); ++i){
		...
		for(int n=0; n<dist_atomt[ii].size(); ++n){
			...
			const int type=struc_train_nnp_[ii].type(m);
			//execute the network
			nnp_.nnh(type).nn().fp(struc_train_nnp_[ii].symm(m));
			//add the atom energy to the total
			struc_train_nnp_[ii].pe()+=nnp_.nnh(type).nn().out()[0];
		}
		//**** accumulate energy across BATCH communicator ****
		MPI_Allreduce(MPI_IN_PLACE,&struc_train_nnp_[ii].pe(),1,MPI_DOUBLE,MPI_SUM,BATCH.mpic());
		//**** compute the energy difference normalized by number of atoms ****
		const double nAtoms=struc_train_ref_[ii].nAtoms();
		const double dE=(struc_train_nnp_[ii].pe()-struc_train_ref_[ii].pe());
		const double dU=dE*normt_[ii];
		double dcdo=eweight_*normt_[ii];
		switch(iter_.loss()){
			case opt::Loss::MSE:{
				error_[0]+=eweight_*0.5*dU*dU;//loss - train
				dcdo*=dU;
		...
		for(int n=0; n<dist_atomt[ii].size(); ++n){
			...
			const int m=dist_atomt[ii].index(n);
			//find the element index in the nn potential
			const int type=struc_train_ref_[ii].type(m);
			//execute the network
			nnp_.nnh(type).nn().fpbp(struc_train_ref_[ii].symm(m));
			//compute the gradient 
			dodp_[type].grad(nnp_.nnh(type).nn());
			gElement_[type].noalias()+=dodp_[type].dodp().row(0)*dcdo;
		}
	}
	...
	if((unsigned int)iter_.step()%iter_.nPrint()==0 || (unsigned int)iter_.step()%iter_.nWrite()==0){
		...
		for(int i=0; i<struc_val_ref_.size(); ++i){
			...
			for(int n=0; n<dist_atomv[i].size(); ++n){
				...
				//find the element index in the nn potential
				const int type=struc_val_ref_[i].type(m);
				//execute the network
				nnp_.nnh(type).nn().fp(struc_val_ref_[i].symm(m));
			...
			//**** compute error ****
			const double nAtoms=struc_val_ref_[i].nAtoms();
			const double dE=(pe-struc_val_ref_[i].pe());
...
void NNPTE::error(const Eigen::VectorXd& x){
	if(NNPTE_PRINT_FUNC>0) std::cout<<"NNPTE::error(const Eigen::VectorXd&):\n";
	...
	for(int bi=0; bi<batcht_.size(); ++bi){
		...
		for(int n=0; n<dist_atomt[si].size(); ++n){
			...
			const int i=dist_atomt[si].index(n);
			//compute error
			df2s+=(struc_train_nnp_[si].force(i)-struc_train_ref_[si].force(i)).squaredNorm(); 
		}
		df2s*=normt_[si]*normt_[si];
		switch(iter_.loss()){
			case opt::Loss::MSE:{
		...
		for(int n=0; n<dist_atomt[si].size(); ++n){
			...
			const Eigen::Vector3d fdiff=(struc_train_nnp_[si].force(i)-struc_train_ref_[si].force(i))*normt_[si];
			...
			for(int j=0; j<nlist_train_[si].size(i); ++j){
				...
				gElement_[II].noalias()+=fweight_*grad_[II]*fdiff.dot(rIJ)*dIJi;
				...
				for(int k=j+1; k<nlist_train_[si].size(i); ++k){
					...
					nnp_.nnh(II).basisA(JJ,KK).forcep(dr,cos);
					gradIJ_[II].setZero();
					gradIK_[II].setZero();
					for(int n=0; n<nnp_.nnh(II).nn().size(); ++n){
						double phi=0; double eta[2]={0,0};
					...
					gElement_[II].noalias()+=fweight_*fdiff.dot(rIJ)*dIJi*gradIJ_[II];
					gElement_[II].noalias()+=fweight_*fdiff.dot(rIK)*dIKi*gradIK_[II];
...
void NNPTE::read(FILE* reader, NNPTE& nnpte){
	...
	while(fgets(input,string::M,reader)!=NULL){
		...
		if(tag=="LOSS"){
			...
			nnpte.eweight()=std::atof(token.next().c_str());
		} else if(tag=="FWEIGHT"){
			nnpte.fweight()=std::atof(token.next().c_str());
		} else if(tag=="ETA"){
		...
		if(WORLD.rank()==0){
			...
			while(fgets(input,string::M,reader)!=NULL){
				...
				if(tag=="COMPUTE"){
					...
					if(ctype=="FORCE") compute.force=string::boolean(token.next().c_str());
