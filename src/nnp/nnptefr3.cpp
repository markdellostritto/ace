...
static bool compare_pair_id(const std::pair<int,double>& p1, const std::pair<int,double>& p2){
	return p1.first<p2.first;
}

static bool compare_pair_is(const std::pair<int,std::string>& p1, const std::pair<int,std::string>& p2){
	return p1.first<p2.first;
}

static bool compare_pair_ii(const std::pair<int,int>& p1, const std::pair<int,int>& p2){
	return p1.first<p2.first;
}
...
int main(int argc, char* argv[]){
	...
	//structures - data
		...
		std::vector<std::vector<int> > natoms(nData);
		std::vector<std::vector<int> > isort(nData);
	...
	try{
		...
		if(WORLD.rank()==0){
			...
			for(int n=0; n<nData; ++n){
				for(int i=0; i<files[n].size(); ++i){
					Structure struc;
					read_struc(files[n][i].c_str(),format,atom,struc);
					natoms[n].push_back(struc.nAtoms());
				}
			}
			...
			for(int n=0; n<nData; ++n){
				if(files[n].size()>0){
					...
					for(int i=0; i<size; ++i){
						fi[n][i].first=natoms[n][i];
						fi[n][i].second=i;
					}
					std::sort(fi[n].begin(),fi[n].end(),compare_pair_ii);
					for(int i=0; i<size; ++i){
						isort[n][i]=fi[n][i].second;
					}
				}
			}
			...
			if(NNPTEFR_PRINT_DATA>1){
				for(int n=0; n<nData; ++n){
					if(fi[n].size()>0){
						std::cout<<print::buf(strbuf)<<"\n";
						std::cout<<print::title("NATOMS",strbuf)<<"\n";
						for(int i=0; i<fi[n].size(); ++i) std::cout<<"\t"<<fi[n][i].first<<" "<<fi[n][i].second<<" "<<files[n][fi[n][i].second]<<"\n";
						std::cout<<print::buf(strbuf)<<"\n";
					}
				}
			}
		...
		for(int n=0; n<nData; ++n) thread::bcast(WORLD.mpic(),0,isort[n]);
		...
		//======== gen indices (equally distribute number of atoms) ========
		for(int n=0; n<nData; ++n){
			indices[n].resize(nstrucs[n],-1);
			int count=0;
			for(int i=0; i<WORLD.size(); ++i){
				for(int j=i; j<indices[n].size(); j+=WORLD.size()){
					indices[n][count++]=isort[n][j];
				}
			}
			...
			if(dist_struc[n].size()>0){
				...
				if(write.energy && WORLD.rank()==0){
					...
					if(writer==NULL) std::cout<<"WARNING: Could not open file: \""<<file<<"\"\n";
						...
						std::sort(energy_r_pair.begin(),energy_r_pair.end(),compare_pair_id);
						std::sort(energy_n_pair.begin(),energy_n_pair.end(),compare_pair_id);
