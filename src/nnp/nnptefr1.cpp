...
void NNPTEFR::train(int batchSize, const std::vector<Structure>& struc_train, const std::vector<Structure>& struc_val){
	...
	for(int n=0; n<struc_train.size(); ++n){
		for(int i=0; i<struc_train[n].nAtoms(); ++i){
			Eigen::VectorXd tmp=1.0/struc_train[n].symm(i).array();
			har_in[struc_train[n].type(i)].noalias()+=tmp;
		}
	}
	for(int i=0; i<nTypes_; ++i){
		har_in[i]/=BATCH.size();//normalize by the size of the BATCH group
		MPI_Allreduce(MPI_IN_PLACE,har_in[i].data(),har_in[i].size(),MPI_DOUBLE,MPI_SUM,WORLD.mpic());
		Eigen::VectorXd tmp=N[i]/har_in[i].array();
		har_in[i]=tmp;
	}
	//compute the average
	if(NNPTEFR_PRINT_STATUS>0 && WORLD.rank()==0) std::cout<<"compute the average\n";
	...
	//weight mask
	Eigen::VectorXd wm=Eigen::VectorXd::Zero(nParams);
...
int main(int argc, char* argv[]){
	...
	//mpi data distribution
		std::vector<thread::Dist> dist_struc(4);
	//timing
	...
	//string
		Token token;
	//random
		unsigned int seed_global=-1;
		std::vector<unsigned int> seed_local;
		std::mt19937 rngen;
	...
	try{
		...
		if(WORLD.rank()==0){
			...
			while(fgets(input,string::M,reader)!=NULL){
				...
				if(tag=="WRITE"){
					...
					else if(wtype=="VDWL") write.vdwl=string::boolean(token.next().c_str());
					else if(wtype=="VDWS") write.vdws=string::boolean(token.next().c_str());
		...
		if(NNPTEFR_PRINT_STATUS>-1 && WORLD.rank()==0) std::cout<<"initializing batch communicator\n";
		...
		dist_struc[0].init(BATCH.ncomm(),BATCH.color(),nstrucs[0]);//train
		dist_struc[1].init(BATCH.ncomm(),BATCH.color(),nstrucs[1]);//validation
		dist_struc[2].init(BATCH.ncomm(),BATCH.color(),nstrucs[2]);//test
		dist_struc[3].init(BATCH.ncomm(),BATCH.color(),nBatch);//batch
		//print
		if(WORLD.rank()==0){
			std::string str;
		...
		for(int n=0; n<nData; ++n){
			if(files[n].size()>0){
				...
				for(int i=0; i<strucs_org[n].size(); ++i){
					thread::bcast(BATCH.mpic(),0,strucs_org[n][i]);
				}
			}
		}
		...
		for(int n=0; n<nData; ++n){
			for(int i=0; i<strucs_org[n].size(); ++i){
				for(int j=0; j<strucs_org[n][i].nAtoms(); ++j){
					strucs_org[n][i].type(j)=nnpte.nnp_.index(strucs_org[n][i].name(j));
				}
			}
		}
		...
		for(int n=0; n<nData; ++n){
			...
			if(strucs_org[n].size()>0){
								...
								for(int j=0; j<nAtoms; ++j){
									...
									switch(perturb){
										...
										case Perturb::COVALENT:{
											const int type=nnpte.nnp_.index(strucs_tot[n][count].name(j));
											const double rcov=nnpte.nnp_.nnh(type).type().rcov().val();
				...
				//broadcast structures to all other procs in the BATCH group
				MPI_Bcast(&nstrucstot,1,MPI_INT,0,BATCH.mpic());
				if(BATCH.rank()!=0) strucs_tot[n].resize(nstrucstot);
				for(int i=0; i<strucs_tot[n].size(); ++i){
					thread::bcast(BATCH.mpic(),0,strucs_tot[n][i]);
				}
			}
		}
		...
		if(compute.coul){
			...
			for(int n=0; n<nData; ++n){
				std::vector<double> ecoul(strucs_org[n].size(),std::numeric_limits<double>::max());
				for(int i=BATCH.rank(); i<strucs_org[n].size(); i+=BATCH.size()){
					NeighborList nlist(strucs_org[n][i],pot_coul.rc());
					ecoul[i]=pot_coul.energy(strucs_org[n][i],nlist);
				}
				MPI_Allreduce(MPI_IN_PLACE,ecoul.data(),ecoul.size(),MPI_DOUBLE,MPI_MIN,BATCH.mpic());
				for(int i=0; i<strucs_org[n].size(); ++i){
					strucs_org[n][i].ecoul()=ecoul[i];
					strucs_org[n][i].pe()-=ecoul[i];
				}
			}
			for(int n=0; n<nData; ++n){
				std::vector<double> ecoul(strucs_tot[n].size(),std::numeric_limits<double>::max());
				for(int i=BATCH.rank(); i<strucs_tot[n].size(); i+=BATCH.size()){
					NeighborList nlist(strucs_tot[n][i],pot_coul.rc());
					ecoul[i]=pot_coul.energy(strucs_tot[n][i],nlist);
				}
				MPI_Allreduce(MPI_IN_PLACE,ecoul.data(),ecoul.size(),MPI_DOUBLE,MPI_MIN,BATCH.mpic());
				for(int i=0; i<strucs_tot[n].size(); ++i){
					strucs_tot[n][i].ecoul()=ecoul[i];
					strucs_tot[n][i].pe()-=ecoul[i];
				}
			}
		}
		...
		if(compute.rep){
			...
			//compute energy
			for(int n=0; n<nData; ++n){
				std::vector<double> erep(strucs_org[n].size(),std::numeric_limits<double>::max());
				for(int i=BATCH.rank(); i<strucs_org[n].size(); i+=BATCH.size()){
					NeighborList nlist(strucs_org[n][i],pot_rep.rc());
					erep[i]=pot_rep.energy(strucs_org[n][i],nlist);
				}
				MPI_Allreduce(MPI_IN_PLACE,erep.data(),erep.size(),MPI_DOUBLE,MPI_MIN,BATCH.mpic());
				for(int i=0; i<strucs_org[n].size(); ++i){
					strucs_org[n][i].erep()=erep[i];
					strucs_org[n][i].pe()-=erep[i];
				}
			}
			for(int n=0; n<nData; ++n){
				...
				for(int i=BATCH.rank(); i<strucs_tot[n].size(); i+=BATCH.size()){
					...
					erep[i]=pot_rep.energy(strucs_tot[n][i],nlist);
				}
				MPI_Allreduce(MPI_IN_PLACE,erep.data(),erep.size(),MPI_DOUBLE,MPI_MIN,BATCH.mpic());
				for(int i=0; i<strucs_tot[n].size(); ++i){
					strucs_tot[n][i].erep()=erep[i];
					strucs_tot[n][i].pe()-=erep[i];
				}
			}
		}
		...
		for(int n=0; n<nData; ++n){
			...
			if(strucs_org[n].size()>0){
				...
				for(int i=BATCH.rank(); i<strucs_org[n].size(); i+=BATCH.size()){
					...
					NeighborList nlist(strucs_org[n][i],nnpte.nnp_.rc());
					NNP::symm(nnpte.nnp_,strucs_org[n][i],nlist);
				}
				MPI_Barrier(BATCH.mpic());
				//bcast symmetry functions
				for(int i=0; i<BATCH.size(); ++i){
					const int root=i;
					for(int j=root; j<strucs_org[n].size(); j+=BATCH.size()){
						thread::bcast(BATCH.mpic(),root,strucs_org[n][j]);
					}
				}
				MPI_Barrier(BATCH.mpic());
			}
		...
		for(int n=0; n<nData; ++n){
			...
			if(strucs_tot[n].size()>0){
				...
				for(int i=BATCH.rank(); i<strucs_tot[n].size(); i+=BATCH.size()){
					if(NNPTEFR_PRINT_STATUS>0) std::cout<<"structure-train["<<i<<"]\n";
					NeighborList nlist(strucs_tot[n][i],nnpte.nnp_.rc());
					NNP::symm(nnpte.nnp_,strucs_tot[n][i],nlist);
				}
				MPI_Barrier(BATCH.mpic());
				//bcast symmetry functions
				for(int i=0; i<BATCH.size(); ++i){
					const int root=i;
					for(int j=root; j<strucs_tot[n].size(); j+=BATCH.size()){
						thread::bcast(BATCH.mpic(),root,strucs_tot[n][j]);
					}
				}
				MPI_Barrier(BATCH.mpic());
			}
		...
		for(int n=0; n<nData; ++n){
			if(dist_struc[n].size()>0){
				...
				for(int i=0; i<dist_struc[n].size(); ++i){
					...
					energy_n[dist_struc[n].index(i)]=NNP::energy(nnpte.nnp_,strucs_org[n][i]);
