...
void BasisR::symm(double dr){
	...
	switch(name_){
		...
		case BasisR::Name::SECH:{
			for(int i=0; i<size_; ++i){
				//symm_[i]=math::special::sech(eta_[i]*(dr-rs_[i]))*cutf;
				const double expf=fmexp(-eta_[i]*(dr-rs_[i]));
				symm_[i]=2.0*expf/(1.0+expf*expf)*cutf;
			}
		} break;
		case BasisR::Name::LOGISTIC:{
			for(int i=0; i<size_; ++i){
				//const double sechf=math::special::sech(eta_[i]*(dr-rs_[i]));
				//symm_[i]=sechf*sechf*cutf;
				const double expf=fmexp(-2.0*eta_[i]*(dr-rs_[i]));
				symm_[i]=4.0*expf/((1.0+expf)*(1.0+expf))*cutf;
			}
		} break;
		case BasisR::Name::TANH:{
			for(int i=0; i<size_; ++i){
				symm_[i]=0.5*(tanh(-eta_[i]*(dr-rs_[i]))+1.0)*cutf;
			}
		} break;
		case BasisR::Name::LOGCOSH:{
			for(int i=0; i<size_; ++i){
				symm_[i]=0.5*log1p(fmexp(-2.0*eta_[i]*(dr-rs_[i])))*cutf;
			}
		} break;
		case BasisR::Name::LOGCOSH2:{
			for(int i=0; i<size_; ++i){
...
double BasisR::force(double dr, const double* dEdG)const{
	...
	switch(name_){
		...
		case BasisR::Name::SECH:{
			for(int i=0; i<size_; ++i){
				/*const double sechf=math::special::sech(eta_[i]*(dr-rs_[i]));
				const double tanhf=math::special::tanh(eta_[i]*(dr-rs_[i]));
		...
		case BasisR::Name::LOGISTIC:{
			for(int i=0; i<size_; ++i){
				...
				const double fexp2=fmexp(-2.0*eta_[i]*(dr-rs_[i]));
		...
		case BasisR::Name::LOGCOSH:{
			for(int i=0; i<size_; ++i){
				const double fexp=fmexp(-2.0*eta_[i]*(dr-rs_[i]));
