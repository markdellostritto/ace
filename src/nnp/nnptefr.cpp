...
Regularization Regularization::read(const char* str){
	if(std::strcmp(str,"LASSO")==0) return Regularization::LASSO;
...
int main(int argc, char* argv[]){
	...
	try{
		...
		//======== start wall clock ========
		if(WORLD.rank()==0) clock_wall.start();
			...
			while(fgets(input,string::M,reader)!=NULL){
				...
				if(tag=="CALC_COUL"){
					...
					while(!token.end()){
						...
						if(stag=="RADIUS"){
							shell.radius=std::atof(token.next().c_str());
						} else if(stag=="AMP"){
							shell.amp=std::atof(token.next().c_str());
						} else if(stag=="REP"){
							shell.rep=std::atof(token.next().c_str());
						}
					}
		...
		for(int n=0; n<nData; ++n){
			for(int i=0; i<dist_struc[n].size(); ++i){
				...
				if(std::fabs(strucl.pe())<math::constants::ZERO) std::cout<<"*********** WARNING: Structure \""<<filename<<"\" has ZERO energy. ***********";
				if(compute.force){
					for(int j=0; j<strucl.nAtoms(); ++j){
						const double force=strucl.force(j).squaredNorm();
						if(std::isinf(force)) std::cout<<"WARNING: Atom \""<<strucl.name(j)<<j<<"\" in \""<<filename<<" has inf force.\n";
						if(force!=force) std::cout<<"WARNING: Atom \""<<strucl.name(j)<<j<<"\" in \""<<filename<<" has nan force.\n";
					}
				}
				for(int j=0; j<strucl.nAtoms(); ++j){
					bool match=false;
		...
		if(compute.cgemm){
			...
			//set the engine
			const int nTypes=types.size()+1;//ntypes + shell type
			Engine engine;
			...
			for(int n=0; n<nData; ++n){
				...
				for(int i=0; i<strucs_org[n].size(); i++){
					...
					std::cout<<"t = "<<t<<" ftot = "<<ftot<<" ezero "<<eZero<<" ecoul "<<eCoul<<" ecgemm "<<eCGem<<" alpha "<<eCalc.coul().alpha()<<" natoms "<<struc.nAtoms()<<"\n";
					//compute the force
					for(int j=0; j<nCores; ++j){
					...
					if(favg>1.0e-12){
						for(int m=0; m<nadd; ++m){
							...
							for(int j=0; j<nAtoms; ++j){
								...
								const double vphi=dista(rngen)*math::constants::PI;
		...
		for(int n=0; n<nData; ++n){
			...
			if(strucs_org[n].size()>0){
				for(int i=0; i<strucs_org[n].size(); i++){
					...
					NeighborList nlist(nnpte.nnp().rc());
					nlist.build(strucs_org[n][i]);
					nnpte.nnp().compute_symm(strucs_org[n][i],nlist);
				}
			}
			clock.stop();
		...
		for(int n=0; n<nData; ++n){
			...
			if(strucs_tot[n].size()>0){
				for(int i=0; i<strucs_tot[n].size(); i++){
					if(NNPTEFR_PRINT_STATUS>0) std::cout<<"structure-train["<<i<<"]\n";
					NeighborList nlist(nnpte.nnp().rc());
					nlist.build(strucs_tot[n][i]);
					nnpte.nnp().compute_symm(strucs_tot[n][i],nlist);
				}
			}
			clock.stop();
			time_symm[n]=clock.duration();
		...
		//======== print the memory ========
		if(NNPTEFR_PRINT_STATUS>-1 && WORLD.rank()==0) std::cout<<"printing memory\n";
		{
			//compute memory (gb)
		...
		for(int n=0; n<nData; ++n){
			for(int i=0; i<strucs_org[n].size(); ++i){
				for(int j=0; j<strucs_org[n][i].nAtoms(); ++j){
					strucs_org[n][i].pe()-=nnpte.nnp().nnh(strucs_org[n][i].type(j)).type().energy();
				}
			}
			for(int i=0; i<strucs_tot[n].size(); ++i){
				for(int j=0; j<strucs_tot[n][i].nAtoms(); ++j){
					strucs_tot[n][i].pe()-=nnpte.nnp().nnh(strucs_tot[n][i].type(j)).type().energy();
				}
			}
		}
		...
		for(int n=0; n<nData; ++n){
			if(dist_struc[n].size()>0){
				...
				if(compute.zero){
					for(int i=0; i<dist_struc[n].size(); ++i){
						for(int j=0; j<strucs_org[n][i].nAtoms(); ++j){
							energy_r[dist_struc[n].index(i)]-=nnpte.nnp().nnh(strucs_org[n][i].type(j)).type().energy();
							energy_n[dist_struc[n].index(i)]-=nnpte.nnp().nnh(strucs_org[n][i].type(j)).type().energy();
						}
					}
				}
			...
			for(int n=0; n<nData; ++n){
				if(dist_struc[n].size()>0){
					...
					for(int i=0; i<dist_struc[n].size(); ++i){
						...
						NeighborList nlist(nnpte.nnp().rc());
						nlist.build(struc);
						nnpte.nnp().compute_force(struc,nlist);
		...
		if(compute.pca){
			...
			for(int i=0; i<nobs; ++i){
				...
				for(int n=0; n<strucs_org[0].size(); ++n){
					...
					const double pe=nnp.compute_energy(strucs_org[0][n]);
