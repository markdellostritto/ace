...
void test_symm_super_crystal(){
	//species
	std::vector<NNH::Type> species(2);
	species[0].name()="Na";
	species[0].mass()=22.990;
	species[0].energy()=-1.68396789;
	species[1].name()="Cl";
	species[1].mass()=35.45;
	species[1].energy()=-2.15869789;
	//nnp
	...
	NeighborList nlist_small(rcut);
	nlist_small.build(struc_small);
	NeighborList nlist_large(rcut);
	nlist_large.build(struc_large);
	//compute symmetry functions
	std::cout<<"initializing symmetry functions\n";
	nnp.init(struc_small);
	nnp.init(struc_large);
	std::cout<<"computing symmetry functions\n";
	nnp.compute_symm(struc_small,nlist_small);
	nnp.compute_symm(struc_large,nlist_large);
	/*
		...
		for(int j=0; j<nlat[1]; ++j){
			for(int k=0; k<nlat[2]; ++k){
				for(int n=0; n<struc_small.nAtoms(); ++n){
					...
					//std::cout<<"lg["<<c<<"] = "<<struc_large.symm(c).transpose()<<"\n";
