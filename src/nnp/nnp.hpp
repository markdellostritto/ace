...
public:
	...
	NNP(){defaults();}
	NNP(const std::vector<NNH::Type>& types){defaults();resize(types);}
	~NNP(){}
	...
	//resizing
		void resize(const std::vector<NNH::Type>& types);
	//calculation
		void init(Structure& struc)const;
		double compute_energy(const Structure& struc);
		void compute_symm(Structure& struc, const NeighborList& nlist);
		void compute_symm(Structure& struc);
		double compute_force(Structure& struc, const NeighborList& nlist);
		double compute_force(Structure& struc);
