...
void read(const char* file, const Atom& atom, Structure& struc){
	const char* func_name="read(FILE*,const Atom&,Structure&)";
	...
	try{
		...
		if(std::fabs(lv.determinant())<math::constants::ZERO) throw std::runtime_error("Invalid lattice vector matrix.");
