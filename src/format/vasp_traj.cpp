...
void read(const char* file, const Interval& interval, const Atom& atom, Trajectory& traj){
	static const char* funcName="read(const char*,const Interval&,Trajectory&)";
	...
	try{
		...
		traj.name()=string::trim(fgets(input,string::M,reader));
		...
		if(str==traj.name()) cell_fixed=false;
		...
		traj.resize(nsteps,natomst,atom);
		...
		if(cell_fixed){
			...
			for(int t=0; t<traj.timesteps(); ++t) static_cast<Cell&>(traj.frame(t)).init(lv);
			//read position
			...
			for(int t=0; t<traj.timesteps(); ++t){
				...
				//initialize lattice vectors
				static_cast<Cell&>(traj.frame(t)).init(s_len*scale*lv);
		...
		if(direct){
			...
			for(int t=0; t<traj.timesteps(); ++t){
				for(int n=0; n<natomst; ++n){
					traj.frame(t).posn(n)=traj.frame(t).R()*traj.frame(t).posn(n);
				}
			}
		} else if(s_len!=1.0){
			for(int t=0; t<traj.timesteps(); ++t){
				for(int n=0; n<natomst; ++n){
					traj.frame(t).posn(n)*=s_len;
				}
			}
		}
			...
			for(int t=0; t<traj.timesteps(); ++t){
				...
				for(int n=0; n<nspecies; ++n){
					for(int m=0; m<natoms[n]; ++m){
						traj.frame(t).name(count++)=names[n];
					} 
				}
			}
		}
		...
		std::cout<<"Trajectory loaded in "<<time<<" seconds.\n";
...
void write(const char* file, const Interval& interval, const Atom& atom, const Trajectory& traj){
	const char* funcName="write(const char*,const Interval&,const Atom&,Trajectory&)";
	if(VASP_PRINT_FUNC>0) std::cout<<NAMESPACE_GLOBAL<<"::"<<NAMESPACE_LOCAL<<"::"<<funcName<<":\n";
	//reading/writing
		FILE* writer=NULL;
		bool error=false;
	...
	try{
		...
		//==== check simulation ====
		if(traj.timesteps()==0) throw std::invalid_argument("Invalid simulation.");
		
		//==== set the number of atoms ====
		const int nAtoms=traj.frame(0).nAtoms();
		...
		for(int i=0; i<nAtoms; ++i){
			...
			for(int j=0; j<species.size(); ++j){
				if(traj.frame(0).name(i)==species[j]){
					index=j; break;
				}
			}
			if(index<0){
				species.push_back(traj.frame(0).name(i));
				natoms.push_back(1);
				nspecies++;
				indices[i]=0;
			} else {
		...
		//==== write the coord ====
		std::string coord;
		if(atom.frac()) coord=std::string("Direct");
		...
		if(cell_fixed){
			...
			for(int i=0; i<3; ++i){
				for(int j=0; j<3; ++j){
					fprintf(writer,"%f ",traj.frame(0).R()(j,i)*s_len);
				}
				fprintf(writer,"\n");
			}
			...
			for(int t=ibeg; t<=iend; ++t){
				...
				for(int n=0; n<nAtoms; ++n){
					Eigen::Vector3d posn;
					if(atom.frac()) posn=traj.frame(t).RInv()*traj.frame(t).posn(indices2[n]);
					else posn=traj.frame(t).posn(indices2[n]);
			...
			for(int t=ibeg; t<=iend; ++t){
				...
				for(int i=0; i<3; ++i){
					for(int j=0; j<3; ++j){
						fprintf(writer,"%f ",traj.frame(t).R()(j,i)*s_len);
					}
					fprintf(writer,"\n");
				}
				for(int i=0; i<nspecies; ++i) fprintf(writer,"%s ",species[i].c_str());
