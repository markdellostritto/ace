...
int main(int argc, char* argv[]){
	...
	try{
		...
		//======== start wall clock ========
		if(WORLD.rank()==0) tbeg=high_resolution_clock::now();
		...
		if(WORLD.rank()==0){
			...
			if(!nnopt.restart()){
				...
				throw std::invalid_argument("Reading restart not implemented.");
			}
		...
		//==== stop wall clock ====
		if(WORLD.rank()==0) tend=high_resolution_clock::now();
		if(WORLD.rank()==0) std::cout<<"duration = "<<duration_cast<duration<double>>(tend-tbeg).count()<<"\n";
