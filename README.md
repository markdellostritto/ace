...
* nnh.hpp     - neural network Hamiltonian
* nnp.hpp     - neural network potential
* type.hpp    - stores atom properties
* nnptefr.hpp - nnp - training - energy and force
...
* iter.hpp         - optimization iterator
* loss.hpp         - loss func
...
* make test_nnp_force - unit test for nnp - force
...
small (~10%) fraction of the training data and should reflect a target of the 
neural network potential.
...
Ideally, one would train the potential using both the total energy of each structure as well as
the forces on each of the atoms.  However, this can be computationally expensive due to the gradients
required to compute the forces at each step.  We therefore follow the approach by Cooper et al. 
(doi: 10.1038/s41524-020-0323-8) where forces are incorporated into training by a Taylor series
expansion of the energy.  New structures are generated from the initial training structures by
randomly perturbing the positions and using a first-order expansion to modify the energy by taking
the product of the forces and the position perturbations.  Thus, we exchange cost in computational
time with the cost in memory for storing the extra perturbed structures.  This is implemented in the
function nnptefr - Neural Network Potetential Training - Energy and Force - Random 
...
* gradient descent (sdg,sdm,nag,adagrad,adadelta,adam,nadam,yogi,nogi)
...
The YOGI algorithm can often yield better results than ADAM, and the NOGI algorithm adds stability
...
* __neuron__ - (string) The name of the neuron activation function. Possible values:
	...
	* __ierf__ - inverse error function
